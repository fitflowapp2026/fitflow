    ...
    function getPlanCashCollected(plan, pkg = null, client = null) {
      if (!plan) return 0;
      const clientObj = client || getClient(plan.clientId);
      const packageObj = pkg || getPackage(plan.packageId);
      if (getClientServiceType(clientObj) === 'free_session' || (plan.planType || plan.type) === 'free_session') return 0;
      const totalPrice = getPlanTotalPrice(plan, packageObj, clientObj);
      const paymentMode = clientObj?.paymentMode || plan.paymentMode || 'single';
      const paymentStatus = clientObj?.paymentStatus || plan.paymentStatus || 'unpaid';
      if (paymentStatus === 'paid') return totalPrice;
      if (paymentMode === 'installments') {
        const total = Math.max(1, Number(clientObj?.installmentsTotal || plan.installmentsTotal || 1));
        const paid = Math.max(0, Math.min(total, Number(clientObj?.installmentsPaid || plan.installmentsPaid || 0)));
        return totalPrice * (paid / total);
      }
      return 0;
    }
    ...
    function getMonthSnapshot(monthDate) {
      ...
      const soldPlans = state.plans.filter(plan => sameMonth(new Date(plan.createdAt || plan.startDate || new Date()), monthDate));
      const revenue = soldPlans.reduce((sum, plan) => {
        const client = getClient(plan.clientId);
        const pkg = getPackage(plan.packageId);
        return sum + getPlanCashCollected(plan, pkg, client);
      }, 0);
...
function applyReportFilter() {
  ...
  if (el.reportFilterBar) {
    ...
    function renderReport() {
      ...
      const monthlyBalance = soldPlans.reduce((sum, plan) => {
        const client = getClient(plan.clientId);
        const pkg = getPackage(plan.packageId);
        return sum + getPlanCashCollected(plan, pkg, client);
      }, 0);
...
function renderClientFormStickySummary() {
  ...
  `;
    ...
    function renderOpenSecondaryViews() {
      ...
      if (document.getElementById('operazioniModalBackdrop')?.classList.contains('open')) renderOperazioniModal();
      if (document.getElementById('atRiskModalBackdrop')?.classList.contains('open')) renderAtRiskModal();
      if (document.getElementById('completamentoModalBackdrop')?.classList.contains('open')) renderCompletamentoModal();
    }
    ...
    el.clientForm.addEventListener('submit', event => {
      ...
      if (currentId) {
        ...
        const oldClientName = getClientFullName(client);
        client.firstName = firstName;
        client.lastName = lastName;
        client.name = name;
        const newClientName = getClientFullName(client);
        ...
        const lessonsToResyncForName = [];
        state.lessons = state.lessons.map(lesson => {
          if (lesson.clientId !== client.id) return lesson;
          const updatedLesson = lesson.planId === plan.id
            ? { ...lesson, duration: pkg.duration, lessonType: lesson.lessonType || serviceType }
            : { ...lesson };
          if (oldClientName !== newClientName) {
            updatedLesson.clientName = newClientName;
            lessonsToResyncForName.push(updatedLesson.id);
          }
